...
def generate_dataset_member_a(tickers, config):
    ...
    if config["data_source_mode"] == 'CSV' and os.path.exists(MASTER_FILE) and os.path.exists(PRICE_FILE):
        ...
        print(f"[{time.strftime('%H:%M:%S')}] [Member A] Batching yfinance download to avoid rate limits (429 Too Many Requests)...")
        ...
        for i in tqdm(range(0, len(tickers), chunk_size)):
            ...
            # using threads=True within small chunks allows fast download but prevents massive DDOS flags

            chunk_data = yf.download(chunk_tickers, ignore_tz=True, start=config["data_start_date"], group_by="ticker", auto_adjust=True, progress=False, threads=True)
            ...
            if not chunk_data.empty:
                ...
                if isinstance(chunk_data.columns, pd.MultiIndex):
                    ...
                    pm = chunk_data.xs('Close', axis=1, level=1).ffill()

                else:

                    pm = pd.DataFrame({chunk_tickers[0]: chunk_data['Close']}).ffill()

                price_chunks.append(pm)

                

            time.sleep(3) # cooldown to avoid IP ban

            

        if len(price_chunks) > 0:

            price_matrix = pd.concat(price_chunks, axis=1)
...
def train_pytorch_embedding_model(master_df, price_matrix, daily_returns, config):
    ...
    for _f in feature_config:
        ...
        if _f in ['hist_momentum', 'hist_volatility']:
            ...
            if _f in embed_features:
                ...
                print(f"  {_f:<35} {'DERIVED':<12} {len(horizons):>6}")
    ...
    for i in range(1, len(valid_years) - 1):
        ...
        for ticker in master_df.index:
            ...
            # Scalar requirement check removed - we calculate historical metrics directly per horizon in the vector builder
            ...
            # Gather multi-horizon trailing return and volatility combinations
            ...
            if "hist_momentum" in feature_config:
                ...
                for h in horizons:
                    ...
                    val = annual_returns[ticker].iloc[start_idx : i+1].mean()

                    x_vec_list.append(val if pd.notna(val) else 0.0)
            ...
            if "hist_volatility" in feature_config:
                ...
                for h in horizons:
                    ...
                    val = annual_vols[ticker].iloc[start_idx : i+1].mean()
            ...
            if valid_sample:
                ...
                sample_years.append(valid_years[i])
    ...
    # --- 3.5 TIME-DECAYED SAMPLE WEIGHTING ---

    half_life = config.get("ml_time_decay_half_life", None)

    if half_life is not None and half_life > 0:

        year_arr = np.array(sample_years, dtype=float)

        y_max = year_arr.max()

        time_weights = np.power(2.0, -(y_max - year_arr) / half_life)

        print(f"[{time.strftime('%H:%M:%S')}] [Member A] Time-Decay ACTIVE: half_life={half_life}yr")

        print(f"   Oldest samples ({int(year_arr.min())}): weight = {time_weights.min():.4f}")

        print(f"   Newest samples ({int(y_max)}): weight = {time_weights.max():.4f}")

        print(f"   Effective sample ratio: {time_weights.sum():.1f} / {len(time_weights)} ({time_weights.sum()/len(time_weights)*100:.1f}%)")

    else:

        time_weights = np.ones(len(X_list))

        print(f"[{time.strftime('%H:%M:%S')}] [Member A] Time-Decay: DISABLED (uniform weighting)")

    

    W_time_tensor = torch.tensor(time_weights, dtype=torch.float32)
    ...
    # Correctly handle columns that might be all NaNs or all zeros

    X_m = torch.nanmean(X_tensor_raw, dim=0)

    X_m = torch.nan_to_num(X_m, nan=0.0) 

    X_s = torch.tensor(np.nanstd(X_tensor_raw.numpy(), axis=0)) + 1e-8

    X_s = torch.where(torch.isnan(X_s), torch.ones_like(X_s), X_s)

    X_tensor = (X_tensor_raw - X_m) / X_s

    X_tensor = torch.nan_to_num(X_tensor, nan=0.0)

    

    dataset = TensorDataset(X_tensor, Y_tensor, W_time_tensor)
    ...
    patience = config.get('ml_early_stop_patience', 3)

    min_delta = config.get('ml_early_stop_delta', 1e-5)
    ...
    for epoch in range(config["ml_epochs"]):
        ...
        for bx, by, bw in loader:
            ...
            # --- 6. DEEP HARDENED MASKED LOSS ---

            # Using torch.where to swap NaNs BEFORE subtraction prevents NaN gradients

            valid_mask = ~torch.isnan(by)

            safe_by = torch.where(valid_mask, by, preds.detach())

            

            error = preds - safe_by

            weighted_error = error * global_w_norm.unsqueeze(0)

            mse_elements = weighted_error ** 2

            

            # Normalize by active weights for this batch
            ...
            sample_loss = (mse_elements * valid_mask.float()).sum(dim=1) / weight_sums.squeeze()

            loss = (sample_loss * bw).sum() / (bw.sum() + 1e-8)

            

            loss.backward()

            # Gradient clipping to prevent explosion on first few steps
        ...
        if epoch % 50 == 0 or epoch == config["ml_epochs"]-1:
            ...
            print(f"   Epoch {epoch}/{config['ml_epochs']} | Masked Weighted Loss: {avg_loss:.4f}")

        

        # Hard NaN check after first epoch to catch early failures

        if epoch == 0:

            if any(torch.isnan(p).any() for p in model.parameters()):

                print("[FATAL] NaN Weights detected after Epoch 0! Aborting training to prevent corruption.")

                break

        

        if abs(prev_loss - avg_loss) < min_delta:

            patience_count += 1

            if patience_count >= patience:

                print(f"   [Early Stop] Loss improvement < {min_delta} for {patience} epochs. Stopping at epoch {epoch}.")
        ...
        for ticker in master_df.index:
            ...
            if "hist_momentum" in feature_config:
                ...
                for h in horizons:
                    ...
                    start_idx = max(0, total_len - h)

                    val = annual_returns[ticker].iloc[start_idx : total_len].mean()
            ...
            if "hist_volatility" in feature_config:
                ...
                for h in horizons:
                    ...
                    val = annual_vols[ticker].iloc[start_idx : total_len].mean()
            ...
            X_m_np = torch.nanmean(X_tensor_raw, dim=0).numpy()

            X_m_np = np.nan_to_num(X_m_np, nan=0.0)

            X_s_np = np.nanstd(X_tensor_raw.numpy(), axis=0) + 1e-8

            X_s_np = np.nan_to_num(X_s_np, nan=1.0) # Avoid div by zero if all NaN

            x_raw_clean = np.nan_to_num(x_raw, nan=0.0)

            x_norm = torch.tensor((x_raw_clean - X_m_np) / X_s_np, dtype=torch.float32).unsqueeze(0)
...
def recommend_and_allocate_member_b(dataset, user_profile, user_dynamic_vector,
    ...
    for ticker, asset_vector in dataset["dynamic_embeddings"].items():
        ...
        if "current_volatility" in master_df.columns:

            val = master_df.loc[ticker, "current_volatility"]
        ...
        # SQUARED RELATIVE PENALTY x10 — bridges 0-10 V_score to 0-100% risk scale

        relative_penalty = (overage / danger_zone) ** 2

        penalty_val = w_vol * relative_penalty * 10  # 10x to match 0-100% risk significance

        

        final_score = (w_sim * sim_score) - penalty_val
